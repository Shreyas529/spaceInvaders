...
def get_user_name():
    while True:
        ...
        old_text=pygame.font.SysFont("bahnschrift",100).render("Enter username",True,(237, 5, 222))
        old_text_rect=old_text.get_rect(center=(510,200))
        SCREEN.blit(old_text,old_text_rect)
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
